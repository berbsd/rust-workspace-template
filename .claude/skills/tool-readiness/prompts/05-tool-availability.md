...
The install path is the repo's setup script. In this template that's `bin/bootstrap` — it installs `betterleaks` and `lefthook` alongside the rest of the toolchain (rustup, `just`, `taplo`, `typos`, `git-cliff`, `cog`). A tool wired into a hook but absent from the setup script works on the machine that added it and nowhere else. (Some repos split provisioning into two scripts — cloud/infra prerequisites versus local dev tooling — if this one grows that split later, this check applies to whichever script installs the hook binaries.)

## Scope

- Every binary named in `lefthook.yml`, CI workflows, `justfile` / `Makefile` / npm scripts
- The repo's setup script(s) — `bin/bootstrap`, `postinstall`, `mise.toml`, `Brewfile`
...
for b in $(grep -hoE '^\s+run:\s*\S+' lefthook.yml | awk '{print $2}' \
  ...
  found=""
  for f in bin/bootstrap Brewfile mise.toml .tool-versions package.json; do
...
Strip the manager from PATH too and you measure nothing — that just returns
`env: lefthook: No such file or directory`, which says only that the test was
wrong, not anything about the hook binary itself.

**lefthook fails CLOSED when a hook's binary is missing.** Since each `run:`
line is a shell command, a missing binary produces the shell's own
`command not found`, a non-zero exit from that command, and lefthook
propagates it — the commit is blocked. Confirm this for the version actually
in use, since manager behavior here is exactly the kind of thing worth not
assuming across versions.
...
1. Ensure the repo has a mechanism that runs `lefthook install`, wired into the
   onboarding step it already has — `bin/bootstrap`, or `package.json`'s
   `prepare` for a pnpm/npm repo. Do not introduce a new script nobody will
   run. This is the fix that matters; the rest is ergonomics.
...
```bash
bin/bootstrap                                  # or the repo's equivalent
...
|---|---|---|---|---|
| betterleaks | yes (`bin/bootstrap`) | yes | fails closed | OK |
| typos | **no** | no | **fails open** | add to bootstrap; guard the hook |
