...
Audit how closed value sets — stages, statuses, kinds, categories — are represented in `migrations/*.sql` and in the Rust types that read them. The platform stores them as `TEXT` constrained by a `CHECK`, with the Rust enum as the type-safe boundary. This check is about that representation, not about whether a given token set is *correct* (that's a domain question) and not about query safety (that's `sql-analyzer`).
...
`services/example` is the exemplar (`example.widgets.stage`, `example.orders.stage`); other services in the workspace follow the same convention — `services/example/migrations/0001_schema.sql` carries the rationale inline.
...
- Tokens are **snake_case** and match the core-model vocabulary exactly (`completed`, never `complete`).
...
- **Token drift from core-model** — a `CHECK` list or enum variant whose wire token disagrees with the `core-model` vocabulary for the same concept.
